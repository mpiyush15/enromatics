 ...
 * Usage: node backend/check-student-by-roll.mjs "2025NE011"
 */
...
async function checkStudentByRoll() {
  try {
    ...
    if (payments.length === 0) {
      ...
      payments.slice(0, 15).forEach((p, i) => {
        const date = p.date ? new Date(p.date).toLocaleDateString() : 'N/A';
        console.log(`${i + 1}. ₹${p.amount || 0} - ${date} (${p.status || 'unknown'}) [${p.method || 'N/A'}]`);
      });
      if (payments.length > 15) {
        console.log(`   ... and ${payments.length - 15} more payments`);
      }
    }
