...
export default function MarksEntryIndexPage() {
  const { user } = useAuth();
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Enter and manage test marks for students</p>
            </div>
            <button
              onClick={fetchTests}
        ...
        {filteredTests.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTests.map((test) => (
              <div
                key={test._id}
                onClick={() => router.push(`/dashboard/client/${tenantId}/academics/marks/${test._id}`)}
                className="bg-white rounded-lg border border-gray-200 hover:border-purple-300 hover:shadow-lg transition-all cursor-pointer group overflow-hidden"
              >
                {/* Card Header */}
                ...
                <div className="p-5 space-y-3">
                  <div>
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                      <span>Total: {test.totalMarks}</span>
                      <span>•</span>
                ...
                <div className="px-5 pb-4">
                  <button className="w-full py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors flex items-center justify-center gap-2 group-hover:bg-purple-700">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                    {test.marksEntered ? 'Edit Marks' : 'Enter Marks'}
                  </button>
                </div>
              </div>
            ))}
