...
export default function OffersManagement() {
  console.log('🎯 [OFFERS PAGE] Component rendering');
  ...
  // Fetch offers
  useEffect(() => {
    console.log('🚀 Offers page mounted, fetching offers...');
    fetchOffers();
  }, [page]);
  ...
  const fetchOffers = async () => {
    try {
      ...
      console.log('📊 Offers fetch response:', data);
      
      if (!res.ok) {
        throw new Error(data.message || `Failed to fetch offers (${res.status})`);
      }
      ...
      console.error('❌ Error fetching offers:', errorMsg);
      setError(errorMsg);
      setOffers([]);
    } finally {
      setLoading(false);
  ...
  // Debug
  console.log('🎯 Offers page render:', { loading, error, offersCount: offers?.length || 0 });
