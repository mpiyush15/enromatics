          ...
          >
            <div className="flex items-start justify-between gap-3 mb-2">
              <p className="font-semibold text-gray-900 dark:text-white text-sm">
                {idx + 1}. {rec.title || rec.message || rec.action}
              </p>
              {rec.priority === "high" && (
            ...
            {rec.actionUrl && (
              <a
                href={rec.actionUrl}
                className="text-xs font-semibold px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition inline-block"
              >
                {rec.button?.label || "View"} →
              </a>
            )}
          </div>
        ))}
